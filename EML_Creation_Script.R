...
# by #####) should be reviewed and edited if necessary,and run one by one. Several sections are labeled as OPTIONAL and
# may only apply to certain data packages. The final section has the make_eml() function to put together a validated EML
# metadata file. Future updates to this script will help bring in additional functions from the EMLeditor (part of the
...
# Vector of dataset URLs - For DataStore we recommend setting this to the main reference page. All data files from a
# single data package can be accessed from the same page so the URLs are the same. This function will repeat the URL you
# provide the appropriate number of times.
data_urls <- c(rep("https://irma.nps.gov/DataStore/Reference/Profile/2293181", length(data_files)))
...
# The next set of optional items are meant to be considered one by one and only run if applicable to a particular
# data package. The first year will typically see all of these run, but if the data format and protocol stays constant
# over time it may be possible to skip some or all of these in future years.
...
# This is where we will continue to improve this EML creation script over time.